        ...
        for item in grupo:
            ...
            with c2:
                if st.button("❌ Rechazar", key=f"rec_{code}", use_container_width=True):
                    ...
                    st.rerun()
            with c3:
                # Mostrar componentes directamente (sin expander anidado)
                if st.button(f"🔍 {n_comp} comp.", key=f"det_{code}", use_container_width=True):
                    st.session_state[f"show_{code}"] = not st.session_state.get(f"show_{code}", False)
                    st.rerun()
                if st.session_state.get(f"show_{code}", False):
                    if n_comp == 0:
                        st.warning("Sin APU — se exportará vacío para completar manualmente.")
                    else:
                        filas = []
                        for sec in SECCIONES:
                            for c in item.get(sec, []):
                                ...
                                filas.append({
                                    "Tipo": sec.replace("_"," ").upper()[:4],
                                    "Descripción": c["description"][:45],
                                    "Und": und,
                                    "Rend": round(rend, 4),
                                    "Unit $": f"{up:,.0f}",
                                    "Parcial $": f"{rend*up:,.0f}",
                                })
                        if filas:
                            import pandas as pd
                            st.dataframe(pd.DataFrame(filas), use_container_width=True, hide_index=True)
                        m1,m2,m3 = st.columns(3)
                        m1.metric("Ofrecido",    f"${precio:,.0f}")
                        m2.metric("Componentes", f"${suma:,.0f}")
